...
import { CancellationError, ValidationError } from "../errors/index.js";
...
/**
 * Fail fast when an optional {@link AbortSignal} has already been aborted.
 *
 * Cooperative checkpoints only — work that started before the abort still
 * completes unless the underlying I/O also honors the signal (HTTP embed does).
 *
 * @param signal - Caller-supplied signal, or `undefined` to skip.
 * @param operation - Facade method name for the error (`remember`, `recall`, …).
 * @throws {ValidationError} When `signal` is provided but is not an AbortSignal.
 * @throws {CancellationError} When `signal.aborted` is true.
 */
export function assertNotAborted(
  ...
  if (typeof AbortSignal === "undefined" || !(signal instanceof AbortSignal)) {
    throw new ValidationError("signal must be an AbortSignal when provided", {
      operation,
      reason: "invalid signal",
      suggestion: "Pass AbortSignal from AbortController, or AbortSignal.timeout(ms)",
    });
  }
  if (signal.aborted) {
    throw new CancellationError(`Operation ${operation}() was cancelled`, {
      operation,
      cause:
