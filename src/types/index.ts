/**
 * Public configuration and domain types for Wolbarg.
 */
...
export interface PostgresDatabaseConfig {
  ...
  url?: string;
  /**
   * Optional max pool size. Defaults to **20** (safer for shared managed Postgres).
   */
  maxPoolSize?: number;
  /** When false, uses asynchronous commit for higher write throughput. */
  durableWrites?: boolean;
  /**
   * Schema owning every Wolbarg table and its NOTIFY channel.
   * Created on open if absent. Defaults to `public`.
   */
  schema?: string;
  /**
   * TLS policy:
   ...
   * - `false` / `"disable"` — force off (logs a warning for remote hosts)
...
export interface RememberOptions {
  ...
  /**
   * Cancel the operation. Checked at cooperative checkpoints and passed to
   * the HTTP embedding client when using the built-in OpenAI-compatible provider.
   */
  signal?: AbortSignal;
}
...
export interface RecallOptions {
  ...
  filter?: MemoryFilter;
  /**
   * When `true`, apply the configured reranker.
   * Throws {@link ValidationError} when no reranker is configured.
   */
  rerank?: boolean;
   ...
   * Throws {@link ValidationError} when no keyword channel is available.
   */
...
export interface MemoryRecord {
   ...
   * SQL-backed records include this value.
   */
  version?: number;
  createdAt: Date;
  updatedAt: Date;
